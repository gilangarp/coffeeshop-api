...
export const cloudinaryArrayUploader = async (
  req: Request,
  prefix: string
): Promise<{ results?: UploadApiResponse[]; errors?: Error[] }> => {
  const files = req.files as Express.Multer.File[];
  if (!files || files.length === 0)
    return { errors: [new Error("No files found")] };
...
export const cloudinarySingleUploader = async (
  req: Request,
  prefix: string,
  id?: string
